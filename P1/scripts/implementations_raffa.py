...
"""
    Pure gradient descend algorithm
"""

def GD(y, tx, grad_n, initial_w, max_iters, gamma):
    ...
    # iterate
    while max_iters > 0 and (np.linalg.norm(w - old_w) / Nx) > eps: 
...
def SGD(y, tx, grad_n, initial_w, max_iters, gamma):
    ...
    # iterate
    while max_iters > 0 and (np.linalg.norm(w - old_w) / Nx) > eps: # TODO add threshold condition
...
"""
def mini_batch_SGD(y, tx, grad_n, L, initial_w, max_iters, gamma):
    ...
    # iterate
    while max_iters > 0 and (np.linalg.norm(w - old_w) / Nx**3) > eps: # TODO add threshold condition

        # generate random integer to consider
        NB = rnd.randint(1,N)
        B = rnd.sample(range(N), NB)
        ...
        # forward step w
        w -= gamma * g
        print(np.linalg.norm(w - old_w) / Nx**2)
        #print("Max iters: ", max_iters)

        # decrement step
        max_iters -= 1

    print(np.linalg.norm(w - old_w) / Nx)
...
def ridge_regression(y, tx, lambda_):
    ...
    wsq = np.dot(np.transpose(w),w)
    
    # element-wise cost-function 
    L_n = lambda yn, txn, w: MSE_cost(yn, txn, w) - lambda_ * N * wsq
    
    return w, compute_cost(y, tx, w, L_n) 
...
def logistic_regression_mb(y, tx, initial_w, max_iters, gamma):
    ...
    grad_n = lambda yn, txn, w: txn * (logistic_sigmoid(np.dot(txn, w)) - yn)

    # compute optimal weight
    w = mini_batch_SGD(y, tx, grad_n, initial_w, max_iters, gamma)
    #w = SGD(y, tx, grad_n, initial_w, max_iters, gamma)
    #w = GD(y, tx, grad_n, initial_w, max_iters, gamma)
